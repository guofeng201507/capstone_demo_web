...
def get_model_log_path(root_path):
    multi_attr_model_dir = os.path.join(root_path, 'img_model')
    may_mkdirs(multi_attr_model_dir)

    multi_attr_log_dir = os.path.join(root_path, 'log')
...
def get_pkl_rootpath(dataset):
    ...
    data_path = os.path.join(root, 'pa_4k_peta_dataset.pkl')
